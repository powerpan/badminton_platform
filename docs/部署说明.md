...
[返回项目简介](../README.md)。下列命令从仓库根目录开始，包含 `cd` 的代码块按各节说明切换目录。

项目完善范围、分阶段验收标准和实施记录见 [项目完善计划](项目完善计划.md)。

当前源码已含四角色、维修只读、前台整段订场/模拟收款/续场；线上订场/商城双渠道及取货已在隔离环境验证，代充值已通过隔离验证，全部订场筛选与关联详情也已通过隔离验证，全局流水、分类汇总与账户核对也已通过隔离验证，全量交付验收仍在进行。旧库需要执行下方阶段八升级，新库使用当前 `sql/init.sql`。设计与实际验证范围见 [软件工程文档索引](软件工程/README.md)。

第二阶段的界面、导航、后台拆分与浏览器验证见 [设计与验收](第二阶段设计与验收.md)。毕业演示可按 [独立演示环境](演示环境.md) 创建新数据库，保留现有业务数据。

第三阶段的推荐、维护、核销、改期、经营统计和输入框修复见 [实施与验收](第三阶段实施与验收.md)，业务术语见 [CONTEXT.md](../CONTEXT.md)。

已有本地开发库的赛事、公告和球友圈内容，可使用 [内容填充与配图](内容填充与配图.md) 中的预览/导入命令补充；与创建完整独立演示库的脚本分开使用。
...
```bash
git clone https://github.com/powerpan/badminton_platform.git
...
当前数据库基础兼容本机 MySQL 5.7.28。用户现有业务库尚未执行此升级。新库初始化含 25 张表；已有阶段七库使用增量工具，不能靠重复导入初始化脚本补列。

先在仓库根目录仅预览：

```bash
backend/.venv/bin/python -B scripts/migrate_staff_payments.py --database badminton_platform
```

预览输出待执行 DDL、原项目表行数与行摘要，不输出账号内容或连接密码。仅在停止业务写入、备份且确认可恢复后执行：

```bash
backend/.venv/bin/python -B scripts/migrate_staff_payments.py --database badminton_platform --apply --writes-stopped
```

如目标是其他库，显式替换库名；连接地址与账号从后端现有环境配置读取。工具不初始化演示账户、不重算旧金额、不将旧封锁自动猜为维修、不生成虚构付款或核销记录。数据库迁移锁仅防止同时运行本工具，不能代替停写。MySQL DDL 不能整体回滚，失败时保存日志和备份，不删除原表重来。

隔离验证可执行 `backend/.venv/bin/python -B backend/tests/integration_staff_foundation.py`，它创建自己命名的测试库并在结束时删除，仅使用结构样例和合成数据。

升级后管理员可以在用户管理中分配前台、维修角色。前台 `/frontdesk` 提供整段开场、模拟收款、续场及未付款撤销；维修 `/maintenance` 显示明确归类的维修安排。工作区已接入线上订场双渠道、改期补差和原渠道退款；商城双渠道、待付库存与取货已接入；前台代充值与管理员全部充值记录已接入，全局流水、分类汇总和订场完整筛选也已接通并通过本地验证；全量回归、设备及实际部署另行验收，不把结构升级当作全部业务上线。已存在新业务数据后，也不可直接回退到仅支持非空客户和余额渠道的旧程序。
