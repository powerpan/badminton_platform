...
export interface CourtBlock extends BookingTarget { id: number; court_no: string; court_name: string; reason: string; status: 'active' | 'released'; block_type: 'maintenance' | 'private_booking' | 'legacy_unspecified' }
export interface BookingSnapshot extends BookingTarget { court_no?: string; court_name?: string }
export interface ReservationChange { id: number; before_snapshot: BookingSnapshot; after_snapshot: BookingSnapshot; balance_change_cents: number; pay_method?: string | null; settlement_delta_cents?: number | null; created_at: string }
export interface OperationsReport {
  ...
  reconciliation: { orders: number; mismatches: number; unverified: number };
...
export const createCourtBlock = (payload: BookingTarget & { reason: string; block_type: string }) => http.post<unknown, ApiResponse<{ id: number }>>('/admin/court-blocks', payload);
...
export interface MaintenanceBlock extends CourtBlock { schedule_status: 'scheduled' | 'in_progress' | 'ended' | 'released' }
export const getMaintenanceBlocks = (params: { date_from: string; date_to: string; court_id?: number; status?: string }) => http.get<unknown, ApiResponse<{ items: MaintenanceBlock[] }>>('/maintenance/court-blocks', { params });
export const classifyCourtBlock = (id: number, block_type: string) => http.put(`/admin/court-blocks/${id}/type`, { block_type });
