...
- 预约创建时写入价格、时长和金额快照，历史预约不受后续场地改价影响。
- 我的预约列表和未开始预约取消。
- 管理员用户新增、启用/禁用、角色修改。
- 管理员场地新增、编辑、启停。
- 管理员预约列表、详情和取消。
...
1. 增加会员账户数据，包含余额、积分、会员等级和有效期。
2. 当前用户资料接口返回会员账户信息，侧栏会员卡读取真实数据。
3. 后台用户管理支持查看和调整会员状态，调整写入操作日志。
4. 预约清单接入会员等级折扣，并展示原价、折扣和应付金额。
5. 创建预约时写入折扣金额和应付金额快照，保证历史预约金额稳定。
6. 同步更新数据库设计、接口设计和当前实现状态文档。

当前第一阶段已完成场地资产、价格和预约金额快照改造。下一阶段应把前端静态会员卡、折扣展示和预约应付金额升级为真实业务数据。
