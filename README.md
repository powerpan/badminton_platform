...
| 用户与会员 | 四角色分配、会员权益与人工调整；模拟收款储值、全部充值记录及到账凭证 |
...
| 角色 | 工作内容 |
| --- | --- |
| 前台 | 散客整段开场、模拟收款、相邻续场、会员代储值与商品取货核销 |
| 维修人员 | 查看明确归类的维修场地、原因、日期和状态，场地恢复由管理员处理 |

前台设备办理收款，散客无需先注册；线上订场保留会员折扣和储值机制。所有支付宝流程均为项目内模拟，不发生真实转账。

## 设计特点

- **连续时段推荐**：使用前缀和筛选完整空闲窗口，依次按开始时间偏差、场地偏好、价格和零散空档排序，返回最多三个候选场次。推荐基于明确规则，提交预约时再次校验可用性。
- **预约与结算一致性**：统一时间区间冲突规则，以 MySQL 行锁和事务保护新流程占用及收退款，Redis 保留既有临时状态职责；金额以整数分计算，改期时在同一事务中处理场次变更、差价和流水。
...
新增的前台、维修角色及模拟支付需求已形成 [软件工程配套文档](docs/软件工程/README.md)，包含需求规格、系统与数据设计、接口、测试验收、实施计划和操作流程。当前已实现四角色权限、前台整段订场与模拟收款、续场和维修只读工作区；线上与商城双渠道、整单取货、代充值及管理员全部订场查询已通过本地隔离验证，全局流水、分类汇总与账户核对也已通过本地隔离验证；全量回归及交付验收继续进行；详见 [开发记录](docs/软件工程/开发进度与验证记录.md)。旧库使用前先完成阶段八升级。
